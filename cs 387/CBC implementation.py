...
def cipher_block_chaining(plaintext, key, init_vec, block_size, block_enc):
    ...
    cipher = []
    start = 0
    end = min(len(plaintext), block_size)
    block = plaintext[start:end]
    cipher.append(block_enc(xor(block, init_vec), key))
    for i in range(1,len(plaintext)/block_size + 1):
        start = i*block_size
        if start >= len(plaintext):
            break
        end = min(len(plaintext), (i+1)*block_size)
        block = plaintext[start:end]
        cipher.append(block_enc(xor(block, cipher[i-1]), key))
    new_cipher = []
...
def xor(plaintext, key):
    ...
    for i in range(len(plaintext)):
        cyphertext.append(1 if plaintext[i] <> key[i] else 0)
    return cyphertext
    # END OF YOUR CODE
    ####################
