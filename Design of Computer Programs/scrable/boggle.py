...
def boggle_words(board, minlength=3):
    ...
    def make_word(pre, path):
        if is_word(pre, minlength):
            words.add(pre)
        if is_pre(pre):
            for j in (neighbors(path[-1], N)):
                if j not in path and not is_border(j):
                    make_word(pre + board[j], path + [j])
...
def is_word(word, minlength=3):
    return word in WORDS and len(word)>=minlength

def is_pre(pre):
    return pre in PREFIXES
