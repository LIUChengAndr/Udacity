...
L_LETTERS = list('abcdefghijklmnopqrstuvwxyz')
...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    i = start + len(pre)
    new_pre = pre.upper()
    if new_pre in WORDS and anchored and not is_letter(row[i]):
        results.add((start, pre))
    if new_pre in PREFIXES:   
        sq = row[i]
        if is_letter(sq):
...
def find_prefixes(hand, pre='', results=None):
    ...
    new_pre = pre.upper()
    if new_pre in WORDS or new_pre in PREFIXES: results.add(pre)
    if new_pre in PREFIXES:
        for L in hand:
            if L == '_':
                for l in L_LETTERS:
