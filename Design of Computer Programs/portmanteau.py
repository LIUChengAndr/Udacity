...
def natalie(words):
    ...
    for word1 in words:
        for word2 in words:
            if word1 <> word2:
                for s,m,e,score in find_natalie(word1,word2):
                    if score > best_score:
                        best_natalie = s+m+e
                        best_score = score
    return best_natalie
                
def word_score(start, mid, end):
    s, m, e = len(start), len(mid), len(end)
    l = s + m + e
    i_s, i_m, i_e = l/4, l/2, l/4
    score = l - abs(s - i_s) - abs(m - i_m) - abs(e - i_e)
    return score

def find_natalie(word1, word2):
    possible = []
    for i in range(len(word1)):
        for j in range(len(word2)):
            if word1[i] == word2[j]:
                len_left = len(word1[i:])
                if word1[i:] == word2[:len_left]:
                    if word1[i:] <> word1 and word2[:len_left] <> word2:
                        possible.append(tuple((word1[:i], word1[i:], 
                                word2[len_left:], word_score(word1[:i], word1[i:], word2[len_left:]))))
    return possible
