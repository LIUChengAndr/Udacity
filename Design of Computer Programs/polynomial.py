...
def add(p1, p2):
    ...
    return poly(tuple(x for x in coefs))
...
def mul(p1, p2):
    ...
    max_len = max((len(p1.coefs), len(p2.coefs)))
    new_coefs = collections.defaultdict(int)
    for i, m in enumerate(p1.coefs):
        for j, n in enumerate(p2.coefs):
            new_coefs[i+j] += m*n
    return poly(tuple(new_coefs[i] for i in range(len(new_coefs))))
