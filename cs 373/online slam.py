...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    t_v = 2*(1 + num_landmarks)
    ordering = [0, 1]+[x for x in range(4,2*(num_landmarks+2))]
    balance = [x for x in range(2,2*(num_landmarks+2))]
    o = blank_grid(t_v,t_v)
    o[0][0] = 1
    o[1][1] = 1
    x = blank_grid(t_v, 1)
    x[0][0] = world_size/2.
    ...
    for i in range(len(data)):
        Omega = Omega.expand(t_v+2, t_v+2, ordering,ordering)
        Xi = Xi.expand(t_v+2, 1, ordering, [0])
        dx,dy = data[i][-1]
        Omega, Xi = make_link(Omega, Xi, 0, 2, dx, t_v+2, motion_noise)
        Omega, Xi = make_link(Omega, Xi, 1, 3, dy, t_v+2, motion_noise)
        for j in range(len(data[i][0])):
            loc, lx, ly = data[i][0][j]
            s_loc = loc - num_landmarks
            Omega, Xi = make_link(Omega, Xi, 0, s_loc*2, lx, t_v+2, measurement_noise)
            Omega, Xi = make_link(Omega, Xi, 1, s_loc*2+1, ly, t_v+2, measurement_noise)
        C = Xi.take([0,1],[0])
        Xi_prime = Xi.take(balance, [0])
        Omega_prime = Omega.take(balance, balance)
        A = Omega.take([0,1], balance)
        A_minus = Omega.take(balance, [0,1])
...
def my_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    t_v = 2*(N + num_landmarks)
    o = blank_grid(t_v,t_v)
    o[0][0] = 1
    o[1][1] = 1
    x = blank_grid(t_v, 1)
    ...
    for i in range(len(data)):
        dx, dy = data[i][-1]
        Omega, Xi = make_link(Omega, Xi, 2*i, 2*i+2, dx, t_v, motion_noise)
        Omega, Xi = make_link(Omega, Xi, 2*i+1, 2*i+3, dy, t_v, motion_noise)
        for j in range(len(data[i][0])):
            loc, lx, ly = data[i][0][j]
            s_loc = loc - num_landmarks
            Omega, Xi = make_link(Omega, Xi, 2*i, s_loc*2, lx, t_v, measurement_noise)
            Omega, Xi = make_link(Omega, Xi, 2*i+1, s_loc*2+1, ly, t_v, measurement_noise)
    mu = Omega.inverse()*Xi
...
def make_link(Omega, Xi, i, j, value, size, noise):
    o_comp = blank_grid(size,size)
    x_comp = blank_grid(size,1)
    o_comp[i][i] = 1/noise
    o_comp[i][j] = -1/noise
    o_comp[j][i] = -1/noise
    o_comp[j][j] = 1/noise
    x_comp[i][0] = -value/noise
    x_comp[j][0] = value/noise
    o = matrix(o_comp)
    x = matrix(x_comp)
    Omega += o
    Xi += x
