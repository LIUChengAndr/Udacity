...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        b_noise = self.bearing_noise
        s_noise = self.steering_noise
        d_noise = self.distance_noise

        alpha = random.gauss(alpha, s_noise)
        d = random.gauss(d, d_noise)
        ...
        if abs(B) > 0.001:
            ...
            cx = x - sin(theta)*R
            cy = y+ cos(theta)*R
            x_prime = cx + sin(theta+B)*R
            y_prime = cy - cos(theta+B)*R
            theta_prime = (theta+B)%(2*pi)
        else:
            x_prime = x + d * cos(theta)
            y_prime = y + d * sin(theta)
            theta_prime = (theta + B)%(2*pi)

        result = robot(length = L)
        result.set(x_prime, y_prime, theta_prime)
        result.bearing_noise = b_noise
        result.steering_noise = s_noise
        result.distance_noise = d_noise
    ...
    def sense(self, add_noise=1): #do not change the name of this function
        ...
        for l in landmarks:
            ...
            a_t = atan2(dy,dx)        
            if add_noise:
                a_t += random.gauss(0.0, self.bearing_noise)
            Z.append(((2*pi)+a_t-self.orientation)%(2*pi))
